...
def get_positive_links(G):
    ...
    for link in tqdm(list(G.edges)):
        for i, pair in enumerate(label_pairs):
            if (
                G.nodes(embed_utils.SENSATTR)[link[0]] == pair[0]
                and G.nodes(embed_utils.SENSATTR)[link[1]] == pair[1]
            ):
                label = i
            elif (
                G.nodes(embed_utils.SENSATTR)[link[1]] == pair[0]
                and G.nodes(embed_utils.SENSATTR)[link[0]] == pair[1]
            ):
                label = i
...
def train_test_from_pos_links(G, pos_links_groups):
    ...
    checked_links = [(link[0], link[1]) for link in train] + [
        (link[1], link[0]) for link in train
    ]
    ...
    for _ in tqdm(range(len(train))):
        ...
        while not_valid:
            ...
            if (
                (node2_ix, node1_ix) in checked_links
                or (node1_ix, node2_ix) in checked_links
                or node1_ix == node2_ix
            ):
                continue
            # Get the correct label
            for i, pair in enumerate(label_pairs):
                if (
                    G.nodes(embed_utils.SENSATTR)[node1_ix] == pair[0]
                    and G.nodes(embed_utils.SENSATTR)[node2_ix] == pair[1]
                ):
                    label = i
                elif (
                    G.nodes(embed_utils.SENSATTR)[node1_ix] == pair[1]
                    and G.nodes(embed_utils.SENSATTR)[node2_ix] == pair[0]
                ):
                    label = i

            # Append negative per corresponding group if not already maximum is achieved
            for group_name, (group_i, max_per_group) in enumerate(
                zip(negative_per_group, maxs_per_group)
            ):
                if label == group_name:
    ...
    checked_links += [(link[0], link[1]) for link in test] + [
        (link[1], link[0]) for link in test
    ]
...
if __name__ == "__main__":
    use_given_dataset = False
    if use_given_dataset:
    ...
    for dataset in datasets:
        ...
        if use_given_dataset:
            ...
            G = generate_graphs.gen_graph(N=200, **dataset)
            r = nx.degree_assortativity_coefficient(G)
            r_a = nx.attribute_assortativity_coefficient(G, embed_utils.SENSATTR)
        ...
        list_disp_min = []
        list_disp_max = []
        for (reweight_method, embed_method) in [
            ("default", "deepwalk"),
            ("fairwalk", "deepwalk"),
            ("crosswalk", "deepwalk"),
        ]:
            ...
            for iter in [str(k) for k in range(1, 6)]:
                ...
                x_train = np.array(
                    [extract_features(emb[l[0]], emb[l[1]]) for l in train]
                )
                ...
                x_test = np.array(
                    [
                        extract_features(np.array(emb[l[0]]), np.array(emb[l[1]]))
                        for l in test
                    ]
                )
                ...
                accuracy["accuracy per iteration"].append(
                    100 * np.sum(y_test == y_pred) / x_test.shape[0]
                )
                ...
                for group in range(len(label_pairs)):
                    ...
                    accuracy_group_x = (
                        100
                        * np.sum(y_test_group_x == y_pred_group_x)
                        / x_test_group_x.shape[0]
                    )
                ...
                results["average weighted accuracy"].append(
                    accuracy_group_xs / x_test.shape[0]
                )
        ...
        if not use_given_dataset:
            print(f"Overal degree assortativity: {r}")
            print(f"Attribute assortativity: {r_a}")
        print(
            f"list of avg accuracies: {list_accs_every_method}\nlist of avg disparities: {list_disp_every_method}"
        )
        list_err_accs = [
            (mean - min, max - mean)
            for mean, min, max in zip(
                list_accs_every_method, list_accs_min, list_accs_max
            )
        ]
        list_err_disp = [
            (mean - min, max - mean)
            for mean, min, max in zip(
                list_disp_every_method, list_disp_min, list_disp_max
            )
        ]
        print(
            f"List of errors accuracies: {list_err_accs}\nList of errors disparities: {list_err_disp}"
        )
