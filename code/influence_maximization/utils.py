...
def plot_influence(influenced_a, influenced_b, num_seeds, filename, population_a, population_b, num_seeds_a,
                   num_seeds_b):
    # total influence
    ...
    plt.plot(np.arange(1, num_seeds + 1),
             np.asarray(list(map(add, influenced_a, influenced_b))) / (population_a + population_b), 'g+')
    plt.xlabel('Number of Seeds')
...
def plot_influence_diff(influenced_a_list, influenced_b_list, num_seeds, labels, filename, population_a, population_b):
    ...
    for i, (influenced_a, influenced_b) in enumerate(zip(influenced_a_list, influenced_b_list)):
        ax.plot(index, (np.asarray(influenced_a) + np.asarray(influenced_b)) / (population_a + population_b),
